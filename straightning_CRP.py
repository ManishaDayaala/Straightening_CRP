...
test_file_path = os.path.join(MAINFOLDER, "24hrData", "Dailydata_straightening.xlsx")  # DAILY DATA
excel_file_path = os.path.join(MAINFOLDER, "Breakdownrecords_straightening.xlsx")  # Recording excel for BD
folderpath = os.path.join(MAINFOLDER, "TemporaryData")  # Temporary dump files collector
threshold_file_path = os.path.join(MAINFOLDER,"Thresholds_StraighteningCRP.xlsx") #
...
def train_ensemble_model(training_file_path, model_folder_path):
    ...
    class_weights = {0: 0.0, 1: 500, 2: 300 }
...
def predict_ensemble(test_file_path, model_folder_path):
    ...
    try:
        ...
        non_zero_codes = [code for code in predicted_labels if "Code 1" in code or "Code 2" in code ]
...
def train_model(training_file_path):
    ...
    def preprocess_data(X, y):
        ...
        joblib.dump(scaler, os.path.join(model_folder_path, 'scalerStraightning_CRP.pkl'))
    ...
    model.save(os.path.join(model_folder_path, 'timeStraightning_CRP.h5'))
...
def predict_time(test_file_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, 'scalerStraightning_CRP.pkl'))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict_time_to_breakdown(X_test_scaled):
        model = load_model(os.path.join(model_folder_path, 'timeStraightning_CRP.h5'))
...
def predict_lstm_autoencoder(test_file_path, model_folder_path):
    ...
    filtered_dict = {sensor: info for sensor, info in sensor_dict.items() if info["count"] > 20}
