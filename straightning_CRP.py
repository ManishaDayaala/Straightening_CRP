...
st.markdown(
    ...
    <div style="display: flex; align-items: center;">
        ...
        <span style="font-size: {title_font_size}px; font-weight: bold;">Breakdown Predictor-Straightening(CRP)</span>
    </div>
...
def process_all_files():
    ...
    required_column_indices = [5, 8, 11, 14, 17]  # F, I, L, O, R
    ...
    st.success(f"Processed {len(excel_files)} files and saved to 'aggregated_output.xlsx'")
...
def predict_lstm_autoencoder(test_file_path, model_folder_path):
    ...
    filtered_dict = {sensor: info for sensor, info in sensor_dict.items() if info["count"] > 10}
